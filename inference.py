...
from utils.utils import myconverter, profiling, sapeon_device_id


def get_default_args(uid: str, num_image: int = 1):
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--model",
        default="models/yolov8s-object-100.onnx",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do profiling.")
    # parser.add_argument(dest='model', type=str, default="../models/yolov8-object.onnx")
    # parser.add_argument(dest='input', type=str, default="./images")
    # parser.add_argument(dest='batch_size', type=int, default=1)
    # parser.add_argument(dest='img_size', nargs='+', type=int, default=[640, 640])
    # parser.add_argument(dest='calib_table', type=str, default="../calib_tables/yolov8s_high.json")
    # parser.add_argument(dest='class_name', type=str, default="../datasets/annotations/class.names")
    # parser.add_argument(dest='dtype', type=str, default='fp32')
    # parser.add_argument(dest='save_img', type=bool, default=True)
    # parser.add_argument(dest='val', type=bool, default=True)
    # parser.add_argument(dest='outputs', type=str, default='./outputs')
    # parser.add_argument(dest='json', type=str, default="./outputs")
    # parser.add_argument(dest='device_id', type=int, default=1)
    # parser.add_argument(dest='num_images', type=int, default=10)
    # parser.add_argument(dest='annotation', type=str, default="../datasets/annotations/instances_mlperf.json")
    # parser.add_argument(dest='conf_thres', type=float, default=0.7)
    # parser.add_argument(dest='iou_thres', type=float, default=0.65)
    # parser.add_argument(dest='profile', type=bool, default=True)
    ...
    args = parser.parse_args([
        ...
        "--dtype", "nf16",
        "--val",
        "--json", f"./api/outputs/{uid}_1.json",
        "--save_img",
        "--output", "./api/outputs",
        "--conf_thres", "0.5",
        "--iou_thres", "0.65",
        "--num_images", f"{num_image}"
    ])
    return args
...
def infer_model(args):
    ...
    print(f"Total Inference Time for Object Detection = {float(sum(latency_list)):.4f} s")
    print(f"Execution Time for Object Detection = {float(exc_time):.4f} s")


def get_default_args_pose(uid: str, num_image: int = 1):
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--class_name",
        default="./datasets/annotations/pose-class.names",
        type=str,
        help="The path of label list.")
    parser.add_argument(
        "--dtype",
...
def infer_model_pose(args):
    ...
    print(f"Total Inference Time for Pose Detection = {float(sum(latency_list)):.4f} s")
    print(f"Execution Time for Pose Detection = {float(exc_time):.4f} s")
