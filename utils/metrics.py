...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        Both sets of boxes are expected to be in (x1, y1, w, h) format.
        Arguments:
...
def convert_cat_id_and_reorientate_bbox(single_annotation):
    cat = int(single_annotation['category_id'])
    bbox = single_annotation['bbox']
    image_id = int(single_annotation['image_id'])
    x1, y1, w, h = bbox
    ...
    single_annotation['image_id'] = image_id
...
def get_class_name(cat, class_name_path):
...
@blockPrinting
def evaluate_on_coco(args, annot):
    # Arguments
    resFile = args.json
    annot_path = annot
    ...
    return {'mAP@[0.5:0.95]': round(cocoEval.stats[0].item(), 3), 'mAP@[0.5]': round(cocoEval.stats[1].item(), 3)}
